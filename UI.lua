...
function ImGui:CreateInstance(Class, Parent, Properties)
    local safeParent = cloneref(Parent or gethui())
    local Instance = cloneref(Instance.new(Class, safeParent))
    ...
    pcall(function()
        Instance.Name = game:GetService("HttpService"):GenerateGUID(false) -- Random unique name
        Instance.Parent = safeParent
    end)
    return Instance
end
...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:CollapsingHeader(Config)
        ...
        Titlebar.Title.Font = Enum.Font.Code
...
function ImGui:CreateWindow(WindowConfig)
	...
	function WindowConfig:SetTitle(Text)
        TitleBar.Left.Title.Font = Enum.Font.Code
